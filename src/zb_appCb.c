 ...
 * FUNCTIONS
 */
int app_bdbNetworkSteerStart(void *arg) {
    printf("app_bdbNetworkSteerStart\r\n");
...
int app_rejoinBacckoff(void *arg) {
...
void zb_bdbInitCb(uint8_t status, uint8_t joinedNetwork) {
    ...
    if (status == BDB_INIT_STATUS_SUCCESS) {
        ...
        if (joinedNetwork) {
            ...
            printf("Factory new, start network steering\r\n");
            uint16_t jitter = 0;
            do {
...
//            printf("No network found, start network steering again\r\n");
