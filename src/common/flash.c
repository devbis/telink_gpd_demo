 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
_attribute_ram_code_sec_ unsigned char app_flash_read_status(unsigned char cmd)
{
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
_attribute_ram_code_sec_ unsigned int app_flash_read_raw_mid(void)
{
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
_attribute_ram_code_sec_ unsigned int app_flash_read_mid(void)
{
	unsigned int flash_mid = 0;
	app_flash_mspi_read_ram(FLASH_GET_JEDEC_ID, 0, 0, 0, (unsigned char*)(&flash_mid), 3);
...
/*
 * The converter runs before the normal application starts copying the image
 * from 0x8000 to 0x0000.  At that point the flash may still have its BP bits
 * set by the factory bootloader.  Do not use the generic 8-bit status write:
 * 8258 flash parts use both 8-bit and 16-bit status layouts, and GD25LD80C
 * and GD25LE80C even share the same JEDEC ID.  Keep this code in RAM because
 * it is called while the flash array is being erased/programmed.
 */
_attribute_ram_code_sec_ static unsigned short app_flash_read_status16(void)
{
	unsigned char low = app_flash_read_status(FLASH_READ_STATUS_CMD_LOWBYTE);
	unsigned char high = app_flash_read_status(FLASH_READ_STATUS_CMD_HIGHBYTE);
	return (unsigned short)low | ((unsigned short)high << 8);
}

_attribute_ram_code_sec_ unsigned char app_flash_unlock_for_ota(void)
{
	unsigned int mid = app_flash_read_mid();

	/* 8-bit status register parts: GD25LD, ZB25WD and MD25D. */
	if ((mid == 0x1060c8) || (mid == 0x13325e) ||
		(mid == 0x134051) || (mid == 0x1360c8) ||
		(mid == 0x14325e) ||
		(mid == 0x1460c8)) {
		unsigned char status = app_flash_read_status(FLASH_READ_STATUS_CMD_LOWBYTE);
		status &= (unsigned char)~0x1c; /* BP[4:2] */
		app_flash_write_status(FLASH_TYPE_8BIT_STATUS, status);
		return (app_flash_read_status(FLASH_READ_STATUS_CMD_LOWBYTE) & 0x1c) == 0;
	}

	/* 16-bit status register parts: GD25LE80C/LQ80C, PUYA and TH. */
	if ((mid == 0x011460c8) || (mid == 0x136085) || (mid == 0x1360eb)) {
		unsigned short status = app_flash_read_status16();
		status &= (unsigned short)~0x407c; /* vendor-specific BP mask */
		app_flash_write_status(FLASH_TYPE_16BIT_STATUS_ONE_CMD, status);
		return (app_flash_read_status16() & 0x407c) == 0;
	}

	/* Never write an unknown status layout blindly. */
	return 0;
}
